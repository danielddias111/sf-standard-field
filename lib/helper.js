...
const getNameFromPluralName = async (parentObject, childObject, url, token) => {
	let singularName
...
const splitClass = (body) => {
	let returnArray = []
	body = body.split('{').join(';').split('}').join(';').split('public').join(';').split('private').join(';').split('protected').join(';').split('global').join(';').split('static').join(';').split('class').join(';').split('void').join(';').split(';')
