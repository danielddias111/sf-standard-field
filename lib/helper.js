...
const getNameFromPluralName = async (mainObj, object, url, token) => {
	let singularName
	let allObjs
	//getNameFromPluralName
	allObjs = await sfRequest.getDescribe(mainObj, url, token)
	allObjs.forEach(obj => {
		if(obj.relationshipName && object === obj.relationshipName.toLowerCase()){
