...
const sfRequest		= require('./salesforceCallouts');
 ...
 */
 const getvariablesToAnalyse = async (symbolTable) => {
	await getAllObjects()
  ...
  let fieldsToAnalyseVarsMap	= analyseVars(symbolTable)
	...
	finalMap = getSalesforceObjects(finalMap)
	
	return finalMap
}

const getSalesforceObjects = (finalMap) => {
	let allObjects 		= myCache.get('AllObjects')
	let mapWithoutSFObjects = new Map()
	for(const [key, value] of finalMap){
		for(let i =0; i < allObjects.length; i++){
			if(key == allObjects[i]){
				mapWithoutSFObjects.set(key, value)
				break
			}
		}
	}
	return mapWithoutSFObjects
}

const getAllObjects = async () => {
	let allObjects 		= myCache.get('AllObjects')
	if(!allObjects){
		allObjects = await sfRequest.getAllObjects()
		const success = myCache.set('AllObjects',allObjects)
	}
	return allObjects
 ...
 const analyseProps = (tempVar) => {
	...
	tempVar.properties.forEach(variable => {
		...
		let curObj 		= String(variable.type).split("<").join('')
																					...
																					.split("List").join('')
																					.split("Map").join('')
			let curObjLower = curObj.toLowerCase()
			if(checkIfPrimativeObject(curObjLower)){
				return //continue
			}
			if(fieldsToAnalyse.get(curObjLower)){
				tempArray = fieldsToAnalyse.get(curObjLower)
			}
			if(!tempArray.includes(variable.name.toLowerCase())){
				tempArray.push(variable.name.toLowerCase())
				fieldsToAnalyse.set(curObjLower, tempArray)
			}
	})
