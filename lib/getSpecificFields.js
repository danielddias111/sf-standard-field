			...
			if(standardArray.length>0){
				for(let i=0; i<standardArray.length; i++){
					if(fields.includes(standardArray[i]) && standardArray[i].split('.').length == 1){
						...
						else if(standardArray[i].split('.').length != 1 && fields.includes(standardArray[i].split('.')[standardArray[i].split('.').length-1])){
							//means it's a field relationship, we need to iterate all fields until the one we want
							let relationshipsList = standardArray[i].split('.')
							//iterate all relationships
							let parentObj = object
							let previousObj = object
							for(let x=0; x<relationshipsList.length-1; x++){
								let tempMap
								...
								for(let allObjFieldsCounter = 0; allObjFieldsCounter<allObjFields.length; allObjFieldsCounter++){
									if(allObjFields[allObjFieldsCounter].relationshipName!= null && allObjFields[allObjFieldsCounter].relationshipName.toLowerCase() == relationshipsList[x]){
										previousObj = parentObj
										parentObj = allObjFields[allObjFieldsCounter].name
										break
									}
								}
								if(x == relationshipsList.length-2){
									let allObjFields = await sfRequest.getDescribe(previousObj,url, token)
									allObjFields.forEach(field => {
										if(field.name == parentObj){
											parentObj = field.referenceTo[0]
										}
									})
									//concating the 2 arrays
									tempArray.push(relationshipsList[relationshipsList.length-1])
									//removing duplicates
									tempArray = [...new Set(tempArray)];
