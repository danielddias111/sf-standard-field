...
async function getSpecificFields(entryPoint, Body,  SymbolTable){
	soqlMap 					= new Map();
	url 							= entryPoint.url
	...
	let fieldsToAnalyse 	= helper.getFieldsToAnalyse(SymbolTable);
	let returnMap 				=  await getSpecificStandardFields(splitedClass, fieldsToAnalyse, SymbolTable)
	return returnMap
}

const getSpecificStandardFields = async (splitedClass, fieldsToAnalyse, SymbolTable) => {
		...
		for(let i=0; i< splitedLine.length; i++){
			...
			if(space.split('.').length!=2){
				...
				for(let c=0; c<varsAndProps.length; c++){
					...
					if(helper.checkIfPrimativeObject(variable.type)){
						continue
					}
